...
order by fechapago desc, direccion
  ...
  async findById(id: number): Promise<Arriendo | null> {
    ...
    async findByCatastroId(catastroid: number): Promise<Arriendo | null> {

    const result = await this.db.query<Arriendo>(
      `SELECT * FROM ${this.tableName} WHERE catastroid = $1 order by fechapago desc`,
      [catastroid]
    );
  ...
  async updateArriendo(id: number, updateData: Partial<Omit<Arriendo, 'id'>>): Promise<Arriendo | null> {
  ...
  async deleteArriendo(id: number): Promise<Arriendo | null> {
